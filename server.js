...
function serveStatic(req, res, pathname){
  ...
  if(routePath === '/' || routePath === '/dashboard') routePath = '/dashboard.html';
  else if(routePath === '/screen') routePath = '/screen.html';
  let filePath = path.join(PUBLIC, routePath.replace(/^\//,''));
...
const server = http.createServer(async (req, res)=>{
  try{
    ...
    if(u.pathname === '/api/stock') return json(res, 200, await getStock(u.searchParams.get('force') === '1'));
    if(u.pathname === '/api/read-car'){
      const carUrl = u.searchParams.get('url') || '';
      if(!carUrl) return json(res, 400, {ok:false, error:'missing url'});
      return json(res, 200, await readCar(carUrl));
    }
...
server.listen(PORT, ()=> console.log('MZJ Showroom Screens v18 running: http://localhost:'+PORT+'  |  /dashboard  /screen?id=A1'));
