...
var THEME_META={
  ramadan:{title:'رمضان',caption:'رمضان يجمعنا',logoDataUrl:'',logoSize:120},
  national:{title:'اليوم الوطني',caption:'اليوم الوطني السعودي',logoDataUrl:'/assets/themes/national-day-default.png',logoSize:120,nationalDayNumber:'96'},
  founding:{title:'يوم التأسيس',caption:'جذورنا تصنع مستقبلنا',logoDataUrl:'',logoSize:120},
  'eid-fitr':{title:'عيد الفطر',caption:'عيدكم أجمل',logoDataUrl:'',logoSize:120},
  'eid-adha':{title:'عيد الأضحى',caption:'عيد أضحى مبارك',logoDataUrl:'',logoSize:120}
};
var themeIdentity=Object.assign({},THEME_META.ramadan);
...
function activeThemeId(){return String(displaySettings.activeThemeId||screenOptions.themeId||'ramadan');}
function themeDefaults(id){return Object.assign({},THEME_META[id]||THEME_META.ramadan);}

function listenThemeIdentity(id){
  if(themeIdentityUnsub){try{themeIdentityUnsub();}catch(e){}themeIdentityUnsub=null;}
  var fallback={};
  if(id==='national'){if(displaySettings.nationalDayLogoDataUrl)fallback.logoDataUrl=displaySettings.nationalDayLogoDataUrl;if(displaySettings.nationalDayNumber)fallback.nationalDayNumber=displaySettings.nationalDayNumber;}
...
function specIcon(key){var icons={engine:'<path d="M4 8h3l2-2h6l2 2h3v8h-2v2h-3v-2H8v2H5v-2H4z"/><path d="M1 10h3m16 2h3m-12-8v2m4-2v2"/>',transmission:'<circle cx="6" cy="6" r="2"/><circle cx="18" cy="6" r="2"/><circle cx="6" cy="18" r="2"/><circle cx="18" cy="18" r="2"/><path d="M6 8v8m12-8v8M8 6h8M8 18h8M12 6v12"/>',drive:'<circle cx="5" cy="6" r="2"/><circle cx="19" cy="6" r="2"/><circle cx="5" cy="18" r="2"/><circle cx="19" cy="18" r="2"/><path d="M7 6h10M7 18h10M5 8v8m14-8v8M12 6v12"/>',fuel:'<path d="M5 3h9v18H5z"/><path d="M8 6h3m3 2h2l3 3v7a2 2 0 0 0 2 2V9l-2-2"/>',seats:'<circle cx="8" cy="5" r="2"/><circle cx="16" cy="5" r="2"/><path d="M4 11c0-2 2-3 4-3s4 1 4 3v7H4zm8 0c0-2 2-3 4-3s4 1 4 3v7h-8"/>',year:'<rect x="3" y="5" width="18" height="16" rx="2"/><path d="M7 3v4m10-4v4M3 10h18M7 14h3m4 0h3M7 18h3"/>',body:'<path d="M3 15l2-5 4-3h7l4 4 1 4v3H3z"/><circle cx="7" cy="18" r="2"/><circle cx="18" cy="18" r="2"/>',trim:'<path d="M4 4h12l4 4-10 12L3 13z"/><circle cx="8" cy="9" r="1.5"/>',hp:'<path d="M4 14a8 8 0 1 1 16 0"/><path d="M12 14l5-5"/><circle cx="12" cy="14" r="2"/>',torque:'<path d="M5 7a8 8 0 1 1-1 9"/><path d="M4 7v5h5"/>',gears:'<circle cx="12" cy="12" r="3"/><path d="M12 2v3m0 14v3M2 12h3m14 0h3M5 5l2 2m10 10 2 2M19 5l-2 2M7 17l-2 2"/>','fuel economy':'<path d="M4 18h16"/><path d="M6 16c2-7 5-10 12-12"/><path d="M12 8l3 3"/>',warranty:'<path d="M12 3l7 3v5c0 5-3 8-7 10-4-2-7-5-7-10V6z"/><path d="M9 12l2 2 4-5"/>',cylinders:'<path d="M6 5h12v14H6z"/><path d="M9 2v3m6-3v3M9 9h6m-6 4h6"/>',length:'<path d="M3 12h18"/><path d="M6 9l-3 3 3 3m12-6 3 3-3 3"/>',width:'<path d="M12 3v18"/><path d="M9 6l3-3 3 3m-6 12 3 3 3-3"/>',height:'<path d="M12 2v20"/><path d="M9 5l3-3 3 3m-6 14 3 3 3-3"/>',wheelbase:'<path d="M3 15h18M6 12l2-5h8l2 5"/><circle cx="7" cy="17" r="2"/><circle cx="17" cy="17" r="2"/>'};var body=icons[key]||'<circle cx="12" cy="12" r="8"/><path d="M12 8v5l3 2"/>';return'<svg viewBox="0 0 24 24" aria-hidden="true">'+body+'</svg>';}
...
function identityHtml(){
  var logo=String(themeIdentity.logoDataUrl||''),size=Math.max(60,Math.min(240,parseInt(themeIdentity.logoSize||120,10))),caption=cleanText(themeIdentity.caption||'');
  var number=activeThemeId()==='national'?cleanText(themeIdentity.nationalDayNumber||'96'):'';
  return '<div class="theme-identity">'+(logo?'<img src="'+esc(logo)+'" alt="" style="width:'+size+'px">':'')+(caption?'<div class="identity-caption">'+esc(caption)+'</div>':'')+(number?'<div class="identity-number">'+esc(number)+'</div>':'')+'</div>';
}
...
function colorsHtml(){var rows=colorRows(),lines=[];if(rows.external.length)lines.push('<div class="color-line"><span>الخارجي</span><div class="color-list">'+rows.external.map(function(r){return colorChip(r,'external');}).join('')+'</div></div>');if(rows.internal.length)lines.push('<div class="color-line"><span>الداخلي</span><div class="color-list">'+rows.internal.map(function(r){return colorChip(r,'internal');}).join('')+'</div></div>');return lines.length?'<div class="colors-strip"><strong>الألوان المتاحة</strong><div class="color-groups">'+lines.join('')+'</div></div>':'';}
function sectionTitle(text,icon){return'<div class="section-title"><i>'+esc(icon||'✦')+'</i><strong>'+esc(text)+'</strong></div>';}
function mainSpecsHtml(){var specs=mainSpecs();if(!specs.length)return'';return'<section class="flow-section">'+sectionTitle('المواصفات الرئيسية','◆')+'<div class="main-specs-grid">'+specs.map(function(x){return'<div class="main-spec">'+specIcon(x[2])+'<small>'+esc(x[0])+'</small><strong>'+esc(x[1])+'</strong></div>';}).join('')+'</div></section>';}
function techHtml(){var groups=technicalGroups();if(!groups.length)return'';if(techIndex>=groups.length)techIndex=0;var g=groups[techIndex];return'<section class="tech-zone">'+sectionTitle('المواصفات الفنية','✦')+'<div class="tech-tabs">'+groups.map(function(x,i){return'<button type="button" class="tech-tab '+(i===techIndex?'active':'')+'" data-tech="'+i+'">'+esc(x.title)+'</button>';}).join('')+'</div><div class="tech-grid">'+g.items.map(function(x){return'<div class="tech-spec">'+specIcon(x[2])+'<small>'+esc(x[0])+'</small><strong>'+esc(x[1])+'</strong></div>';}).join('')+'</div></section>';}
function pagerHtml(total){if(total<=1)return'';return'<div class="feature-pager"><button type="button" class="feature-page-btn" data-step="-1">‹</button><strong>'+(featurePage+1)+' / '+total+'</strong><button type="button" class="feature-page-btn" data-step="1">›</button></div>';}
function featuresHtml(){var groups=featureData(),valid=validFeatureSections();if(!valid.length)return'';if(valid.indexOf(featureSection)<0){featureSection=valid[0];featurePage=0;}var items=groups[featureSection]||[],pages=Math.max(1,Math.ceil(items.length/14));if(featurePage>=pages)featurePage=0;if(featurePage<0)featurePage=pages-1;var visible=items.slice(featurePage*14,(featurePage+1)*14),right=visible.slice(0,7),left=visible.slice(7,14);function rows(arr){return arr.map(function(x){return'<div class="feature-row"><i>✓</i><span>'+esc(x)+'</span></div>';}).join('');}return'<section class="features-zone"><div class="feature-tabs">'+valid.map(function(k){return'<button type="button" class="feature-tab '+(k===featureSection?'active':'')+'" data-section="'+k+'">'+esc(featureTitle(k))+'</button>';}).join('')+'</div><div class="feature-columns"><div class="feature-column">'+rows(right)+'</div><div class="feature-column">'+rows(left)+'</div></div>'+pagerHtml(pages)+'</section>';}
function bottomHtml(){return'<footer class="bottom-bar"><div class="qr-side"><div id="showroomQr" class="qr-box"></div><div class="qr-copy"><strong>تفاصيل السيارة على جوالك</strong><span>امسح الرمز لعرض التفاصيل الكاملة</span></div></div><div class="tagline">رحلة أجمل .. مع MZJ</div></footer>';}
...
function render(){
  ...
  root.innerHTML='<section class="visual-zone">'+identityHtml()+'<div class="car-portal"><div class="car-window"><img id="showroomCarImage" loading="eager" decoding="async" fetchpriority="high" alt="'+esc(title||'صورة السيارة')+'"></div><div class="portal-ribbon"></div></div><div class="image-controls"><button id="imgPrev" class="prev" type="button">‹</button><button id="imgNext" class="next" type="button">›</button></div><div id="showroomDots" class="image-dots"></div></section>'+
  '<aside class="info-zone"><div class="info-canvas"><header class="topline"><div class="vehicle-title-wrap">'+mzjLogoHtml()+'<h1 class="vehicle-title">'+esc(title)+'</h1></div>'+(price?'<div class="price-medallion"><strong>'+esc(price)+' <span>ر.س</span></strong><small>السعر شامل ضريبة القيمة المضافة</small></div>':'')+'</header>'+colorsHtml()+mainSpecsHtml()+'<div id="techSlot">'+techHtml()+'</div><div id="featureSlot">'+featuresHtml()+'</div>'+bottomHtml()+'</div></aside>';
  applyDisplaySettings();bindRendered();updateImageOnly();renderQr();startImageLoop();startFeatureLoop();
}

function renderQr(){var el=document.getElementById('showroomQr'),url=screenOptions.carUrl||(data&&data.carUrl)||'';if(!el||!url)return;el.innerHTML='';if(window.QRCode){try{new QRCode(el,{text:url,width:70,height:70,colorDark:'#201915',colorLight:'#ffffff',correctLevel:QRCode.CorrectLevel.M});}catch(e){}}}
function renderDots(){var el=document.getElementById('showroomDots'),arr=images();if(!el)return;if(arr.length<2){el.innerHTML='';el.classList.add('hidden');return;}el.classList.remove('hidden');var current=imageIndex%arr.length,max=Math.min(arr.length,14);el.innerHTML=Array.from({length:max}).map(function(_,i){return'<span class="image-dot '+(i===Math.min(current,max-1)?'active':'')+'"></span>';}).join('');}
function updateImageOnly(){var img=document.getElementById('showroomCarImage'),src=currentImage();if(img){if(src){img.dataset.originalSrc=src;img.dataset.directTried='0';var compatible=compatibleImageUrl(src);if(img.getAttribute('src')!==compatible)img.src=compatible;img.classList.remove('hidden');}else{img.removeAttribute('src');img.classList.add('hidden');}}renderDots();renderDebug();}
...
function handleImageError(img){var original=img&&img.dataset?String(img.dataset.originalSrc||''):'';if(original&&img.dataset.directTried!=='1'){img.dataset.directTried='1';try{var directUrl=new URL(original,location.href);directUrl.searchParams.set('_mzjcb',PAGE_CACHE_BUSTER);img.src=directUrl.toString();}catch(e){img.src=original+(original.indexOf('?')>=0?'&':'?')+'_mzjcb='+encodeURIComponent(PAGE_CACHE_BUSTER);}return;}if(original)badLoaded[original]=1;var arr=images();imageIndex=arr.length?Math.min(imageIndex,arr.length-1):0;updateImageOnly();}
function renderDebug(){if(!/[?&]debug=1(?:&|$)/.test(location.search))return;var el=document.getElementById('mzjDebug');if(!el){el=document.createElement('div');el.id='mzjDebug';el.className='debug';document.body.appendChild(el);}el.textContent='Screen '+screenId+' | images '+images().length+' | index '+imageIndex+' | viewport '+innerWidth+'x'+innerHeight+' | '+(currentImage()||'NO IMAGE');}
